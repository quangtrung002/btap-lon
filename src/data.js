const data = {
  iphone: [
    {
      id: "1",
      name: "Iphone 13 Pro Max",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/04/iPhone-13-pro-max.png",
      price: "29.290.000",
      Capacity: ["128GB", "256GB", "512GB", "1TB"],
      color: ["Alpine Green", "Sierra Blue", "Graphite", "Silver", "Gold"]
    },
    {
      id: "2",
      name: "Iphone 13",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/04/iPhone-13.png",
      price: "19.990.000",
      Capacity: ["128GB", "256GB", "512GB"],
      color: ["Alpine Green", "Midnight", "Starlight", "Blue", "(PRODUCT) RED", "Pink"]
    },
    {
      id: "3",
      name: "Iphone 12",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/04/iPhone-12-xanh.png",
      price: "16.190.000",
      Capacity: ["64GB", "128GB", "256GB"],
      color: ["White", "Black", "Blue", "Green", "Purple", "(PRODUCT) RED"]
    },
    {
      id: "4",
      name: "Iphone 11",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/04/iPhone-11-Green.png",
      price: "11.690.000",
      Capacity: ["64GB", "128GB", "256GB"],
      color: ["White", "Black", "Yellow", "Green", "Purple", "(PRODUCT) RED"]
    },
    {
      id: "5",
      name: "iPhone 13 mini",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/04/ip13mini-1.png",
      price: "18.590.000",
      Capacity: ["128GB", "256GB", "512GB"],
      color: ["Alpine Green", "Midnight", "Starlight", "Blue", "(PRODUCT) RED", "Pink"]
    },
    {
      id: "6",
      name: "iPhone 13 Pro",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/04/ip13pro-1.png",
      price: "26.990.000",
      Capacity: ["128GB", "256GB", "512GB", "1TB"],
      color: ["Alpine Green", "Sierra Blue", "Graphite", "Silver", "Gold"]
    },
    {
      id: "7",
      name: "iPhone 12 mini",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/04/ip12mini.png",
      price: "14.950.000",
      Capacity: ["64GB", "128GB", "256GB"],
      color: ["White", "Black", "Blue", "Green", "Purple", "(PRODUCT) RED"]
    },
    {
      id: "8",
      name: "iPhone SE (2022)",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/04/ipse.png",
      price: "11.990.000",
      Capacity: ["64GB", "128GB", "256GB"],
      color: ["Midnight", "Starlight", "Blue", "(PRODUCT) RED"]
    },
  ],
  ipad : [
    {
      id: "1",
      name: "iPad gen 9 (2021)",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/03/ipad-gen-9.png",
      price: "8.990.000",
      Capacity: ["64GB", "256GB"],
      color: ["Space Gray", "Silver"],
      Model: ["Wi-Fi", "Wi-Fi + 4G"]
    },
    {
      id: "2",
      name: "iPad mini 6 (2021)",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/03/ipad-mini-6.png",
      price: "13.290.000",
      Capacity: ["64GB", "256GB"],
      color: ["Starlight", "Space Gray", "Purple", "Pink"],
      Model: ["Wi-Fi", "Wi-Fi + 5G"]
    },
    {
      id: "3",
      name: "iPad Pro 11 inch (M1, 2021)",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/11/ipad-pro-m1-11-768x768-1.webp",
      price: "19.950.000",
      Capacity: ["128GB", "256GB", "512GB", "1TB", "2TB"],
      color: ["Space Gray", "Silver"],
      Model: ["Wi-Fi", "Wi-Fi + 5G"]
    },
    {
      id: "4",
      name: "iPad Pro 12.9 inch (M1, 2021)",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/11/ipad-pro-m1.12.9-768x768-1.webp",
      price: "24.990.000",
      Capacity: ["128GB", "256GB", "512GB", "1TB", "2TB"],
      color: ["Space Gray", "Silver"],
      Model: ["Wi-Fi", "Wi-Fi + 5G"]
    },
    {
      id: "5",
      name: "iPad Air 5",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/11/ipad-pro-m1.12.9-768x768-1.webp",
      price: "15.390.000",
      Capacity: ["64GB", "256GB"],
      color: ["Starlight", "Space Gray", "blue", "Purple", "Pink"],
      Model: ["Wi-Fi", "Wi-Fi + 5G"]
    },
    {
      id: "6",
      name: "iPad Air 4",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/03/ipad-air-4.png",
      price: "14.490.000",
      Capacity: ["64GB", "256GB"],
      color: ["Space Gray", "silver", "Green", "Sky blue", "Pink"],
      Model: ["Wi-Fi", "Wi-Fi + 4G"]
    },
  ],
  mac : [
    {
      id: "1",
      name: "MacBook Pro 14 M1 Pro (14-Core/16GB/512GB)",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/10/MacBook_Pro_14-in_Silver-1.png",
      price: "47.990.000",
      color: ["Space Gray", "silver"]
    },
    {
      id: "2",
      name: "MacBook Pro 14 M1 Pro (16-Core/16GB/1TB)",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/10/MacBook_Pro_14-in_Silver-1.png",
      price: "58.990.000",
      color: ["Space Gray", "silver"]
    },
    {
      id: "3",
      name: "MacBook Air M1 2020",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/10/MacBook-Air-M1-2020.png",
      price: "23.490.000",
      color: ["Space Gray", "silver", "Gold"]
    },
    {
      id: "4",
      name: "MacBook Pro M1 2020",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/10/MacBook-Pro-M1-2020.png",
      price: "30.250.000",
      color: ["Space Gray", "silver"]
    },
    {
      id: "5",
      name: "MacBook Pro 16 M1 Pro (16-Core/32GB/512GB)",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/03/MacBook_Pro_16-in_Silver-1.png",
      price: "71.950.000",
      color: ["Space Gray", "silver"]
    },
    {
      id: "6",
      name: "MacBook Pro 16 M1 Pro (16-Core/16GB/1TB)",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/03/MacBook_Pro_16-in_Silver-1.png",
      price: "64.750.000",
      color: ["Space Gray", "silver"]
    },
    {
      id: "7",
      name: "MacBook Pro 14 M1 Max",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/03/MacBook_Pro_14-in_Space-1.png",
      price: "71.990.000",
      color: ["Space Gray", "silver"],
      Capacity: ["512GB", "1TB"],
      Ram: ["32GB", "64GB"],
      CPU: ["24-core", "32-core"]
    },
    {
      id: "8",
      name: "MacBook Pro 16 M1 Max",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/03/MacBook_Pro_16-in_Space_Gray_PDP_Image_Position-1__VN-2-1.png",
      price: "89.950.000",
      color: ["Space Gray", "silver"],
      Capacity: ["1TB"],
      Ram: ["32GB", "64GB"],
      GPU: ["32-core"]
    },
    {
      id: "9",
      name: "MacBook Pro 16 M1 Pro (16-Core/32GB/512GB)",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/03/MacBook_Pro_16-in_Silver-1.png",
      price: "71.950.000",
      color: ["Space Gray", "silver"],
    },
    {
      id: "10",
      name: "MacBook Pro 14 M1 Pro (16-Core/32GB/512GB)",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/10/MacBook_Pro_14-in_Silver-1.png",
      price: "63.950.000",
      color: ["Space Gray", "silver"],
    },
    {
      id: "11",
      name: "MacBook Pro 14 M1 Pro (16-Core/32GB/1TB)",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/10/MacBook_Pro_14-in_Silver-1.png",
      price: "72.790.000",
      color: ["Space Gray", "silver"],
    },
    {
      id: "12",
      name: "MacBook Pro 14 M1 Pro (14-Core/32GB/512GB)",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/10/MacBook_Pro_14-in_Silver-1.png",
      price: "57.450.000",
      color: ["Space Gray", "silver"],
    },
    {
      id: "13",
      name: "Studio Display",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/03/mac-studio-1-removebg-preview-1-1.png",
      price: "43.990.000",
      screen: "Standard Glass"
    },
    {
      id: "14",
      name: "Mac Studio",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/03/macstudio.png",
      price: "59.990.000",
      screen: "Standard Glass",
      Capacity: ["512GB", "1TB"],
      Ram: ["32GB", "64GB"],
      GPU: ["24-core", "48-core"]
    },
    {
      id: "15",
      name: "iMac M1 2021 24 inch",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/10/imac-m1-2021.2.png",
      price: "31.450.000",
      color: ["Silver", "Blue", "Green", "Pink"],
      Capacity: ["256GB", "512GB"],
      Ram: ["8GB", "16GB"],
      Chip: ["7-Core GPU", "8-Core GPU"]
    },
    {
      id: "16",
      name: "iMac 2020",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/07/iMac-2020.png",
      price: "24.550.000",
      Screensize: ["21.5 inch", "27 inch"],
      Capacity: ["256GB", "512GB", "1TB"],
      Screenresolution: ["Full HD", "4K", "5K"],
      Chip: ["Core i3", "Core i5", "Core i7"]
    }
  ],
  airpods : [
    {
      id: "1",
      name: "AirPods 2",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/11/AirPods-2-ShopDunk.png",
      price: "2.750.000",
      oldprice: "4.390.000",
    },
    {
      id: "2",
      name: "AirPods (gen 3)",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/11/AirPods_3rd_Gen_PDP_Image_Position-2__VN-removebg-preview.png",
      price: "4.450.000",
      oldprice: "5.490.000",
    },
    {
      id: "3",
      name: "AirPods Max",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/11/AirPods-Max-1.png",
      price: "10.490.000",
    },
    {
      id: "4",
      name: "AirPods Pro (2021)",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/11/airpods_pro_2021.png",
      price: "4.650.000",
      oldprice: "5.790.000",
    },
    {
      id: "5",
      name: "Beats EP On-Ear Headphones",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/01/13994_tai_nghe_beats_ep_on_ear_headphones_red_3.jpg",
      price: "2.990.000",
      oldprice: "3.290.000",
      color: ["white", "black", "blue", "(PRODUCT) RED"],
    },
    {
      id: "6",
      name: "Beats Fit Pro True Wireless Earbuds",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/01/MK2H3.jpg",
      price: "4.490.000",
      oldprice: "5.990.000",
      color: ["Space Gray", "White", "Black", "Purple"],
    },
    {
      id: "7",
      name: "Beats Solo3 Wireless Headphones",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/01/MX442-1.jpg",
      price: "4.100.000",
      oldprice: "4.990.000",
      color: ["Rose Gold", "Black", "(PRODUCT) RED"],
    },
    {
      id: "8",
      name: "Beats Studio3 Wireless Headphones",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/01/MXJ92.jpg",
      price: "5.990.000",
      oldprice: "6.990.000",
      color: ["Space Gray", "Black", "(PRODUCT) RED"],
    },
    {
      id: "9",
      name: "Beats Studio3 Wireless Over‑Ear Headphones",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/01/MX3Y2.jpg",
      price: "6.990.000",
      oldprice: "7.490.000",
      color: ["White", "Black", "Blue", "(PRODUCT) RED"],
    }
  ],
  watch : [
    {
      id: "1",
      name: "Apple Watch Series 7 Thép",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/10/watch-7-thep-1.png",
      price: "17.990.000",
      oldprice: "21.990.000",
      color : ["Graphite","Silver","Gold"],
      Screensize : ["41mm","45mm"],
      Model : ["GPS + Cellular"],
      Linen : ["Dây thép","Dây thể thao"],
    },
    {
      id: "2",
      name: "Apple Watch Series 7 Nhôm",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/10/watch-7-nhom-1.png",
      price: "9.590.000",
      oldprice: "11.990.000",
      color : ["Midnight","Starlight","Blue","Green","(PRODUCT) RED"],
      Screensize : ["41mm","45mm"],
      Model : ["GPS","GPS + Cellular"],
      Linen : ["Dây thể thao"],
    },
    {
      id: "3",
      name: "Apple Watch Series 6 Thép",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/10/watch-6-thep-1.png",
      price: "16.690.000",
      oldprice: "19.990.000",
      color : ["Graphite","Silver","Gold"],
      Screensize : ["40mm","44mm"],
      Model : ["GPS + Cellular"],
      Linen : ["Dây thép","Dây thể thao"],
    },
    {
      id: "4",
      name: "Apple Watch Series 6 Nhôm",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/10/watch-6-thep-1.png",
      price: "7.490.000",
      oldprice: "11.990.000",
      color : ["Space Gray","Silver","Gold","Blue","(PRODUCT) RED"],
      Screensize : ["40mm","44mm"],
      Model : ["GPS","GPS + Cellular"],
      Linen : ["Dây thể thao"],
    },
    {
      id: "5",
      name: "Apple Watch SE",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/10/watch-6-thep-1.png",
      price: "6.890.000",
      oldprice: "8.990.000",
      color : ["Space Gray","Silver","Gold"],
      Screensize : ["40mm","44mm"],
      Model : ["GPS","GPS + Cellular"],
      Linen : ["Dây thể thao"],
    },
    {
      id: "6",
      name: "Apple Watch Series 3",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/10/watch-3.png",
      price: "4.990.000 ",
      oldprice: "5.990.000",
      color : ["Space Gray","Silver"],
      Screensize : ["38mm","42mm"],
      Model : ["GPS","GPS + Cellular"],
    },
  ],
  accessory : [
    {
      id: "1",
      name: "Combo Ốp – Cường lực Jinya space protecting iPhone 12",
      category: "bao da",
      brand: "apple",
      image: "https://shopdunk.com/wp-content/uploads/2022/04/combo-op-lung-cuong-luc.1.jpg",
      price: "",
      oldprice: "250.000",
      sale: ""
    },
    {
      id: "2",
      name: "Smart Folio for iPad Air (5th generation)",
      category: "bao da",
      brand: "iPad",
      image: "https://shopdunk.com/wp-content/uploads/2022/03/MNA63.jpg",
      price: "2.490.000",
      oldprice: "",
      sale: "10%"
    },
    {
      id: "3",
      name: "Bao da Ducis iPad Mini 6 8.3 inch",
      category: "bao da",
      brand: "ducis",
      image: "https://shopdunk.com/wp-content/uploads/2021/12/bao-da-ipad-mini-6-dux-ducis.jpg",
      price: "300.000",
      oldprice: "350.000",
      sale: "16%"
    },
    {
      id: "4",
      name: "Bao da Tomtoc (USA) từ tính đa góc Vertical For Ipad Pro 12.9-inch 2021",
      category: "bao da",
      brand: "iPad",
      image: "https://shopdunk.com/wp-content/uploads/2022/03/b0…caled_67cfae88f0d3455dae56100c69714f2c_master.jpg",
      price: "",
      oldprice: "1.390.000",
      sale: ""
    },
    {
      id: "5",
      name: "Bao da Smart Cover for iPad (9th generation)",
      category: "bao da",
      brand: "iPad",
      image: "https://shopdunk.com/wp-content/uploads/2022/02/MJM83.jpg",
      price: "2.290.000",
      oldprice: "",
      sale: "15%"
    },
    {
      id: "6",
      name: "Ốp Rockrose Veill II Airpod Pro",
      category: "bao da",
      brand: "iPod",
      image: "https://shopdunk.com/wp-content/uploads/2022/01/op-rockrose-veil-iii-airpod-pro-jpg-a.jpg",
      price: "170.000",
      oldprice: "300.000",
      sale: '43%'
    },
    {
      id: "7",
      name: "Ốp lưng iPhone 13 Pro Clear Case with MagSafe",
      category: "bao da",
      brand: "devia",
      image: "https://shopdunk.com/wp-content/uploads/2021/12/MM2Y3_AV3.jpg",
      price: "1.290.000",
      oldprice: "1.590.000",
      sale: "19%"
    },
    {
      id: "8",
      name: "Ốp ESR Cloud Soft iPhone 13/13 Pro Max – Pine Green",
      category: "bao da",
      brand: "esr",
      image: "https://shopdunk.com/wp-content/uploads/2022/03/8.png",
      price: "350.000",
      oldprice: "495.000",
      sale: "29%"
    },
    {
      id: "9",
      name: "Miếng dán Paper Like Film Esr iPad – Pro 12.9 2020 – Clear",
      category: "cường lực bảo vệ",
      brand: "esr",
      image: "https://shopdunk.com/wp-content/uploads/2022/05/Untitled-1.png",
      price: "350.000",
      oldprice: "480.000",
      sale: "27%"
    },
    {
      id: "10",
      name: "Miếng dán Paper Like Film Esr iPad – Pro 11 2020 – Clear",
      category: "cường lực bảo vệ",
      brand: "esr",
      image: "https://shopdunk.com/wp-content/uploads/2022/05/Untitled-3.png",
      price: "350.000",
      oldprice: "450.000",
      sale: "22%"
    },
    {
      id: "11",
      name: "Miếng dán màn hình Mocoll New Macbook Pro 13 inch",
      category: "cường lực bảo vệ",
      brand: "MOCOLL",
      image: "https://shopdunk.com/wp-content/uploads/2021/12/mi…-man-hinh-cho-macbook-pro-13-inch-mocoll-2167.jpg",
      price: "",
      oldprice: "299.000",
      sale: ""
    },
    {
      id: "12",
      name: "Miếng dán cường lực Supitech iPhone XR/11",
      category: "cường lực bảo vệ ",
      brand: "supitec",
      image: "https://shopdunk.com/wp-content/uploads/2022/03/cuong-luc.png",
      price: "",
      oldprice: "160.000",
      sale: ""
    },
    {
      id: "13",
      name: "Miếng dán cường lực Supitec iPhone 13/13Pro",
      category: "cường lực bảo vệ",
      brand: "supitec",
      image: "https://shopdunk.com/wp-content/uploads/2022/03/cuong-luc.png",
      price: "190.000",
      oldprice: "200.000",
      sale: "5%"
    },
    {
      id: "14",
      name: "Miếng dán màn hình Wiwu Miếng dán màn hình Macbook 13 inch",
      category: "cường lực bảo vệ",
      brand: "WiWu",
      image: "https://shopdunk.com/wp-content/uploads/2021/12/mi…-ba-o-ve-ma-n-hi-nh-macbook-wiwu-taovang-vn-3.jpg",
      price: "",
      oldprice: "300.000",
      sale: ""
    },
    {
      id: "15",
      name: "Dây đeo Sprectrum Strap Apple Watch Itskins",
      category: "dây đeo Apple Watch",
      brand: "itskins",
      image: "https://shopdunk.com/wp-content/uploads/2021/08/i11.png",
      price: "499.000",
      oldprice: "",
      sale: ""
    },
    {
      id: "16",
      name: "Dây đeo Watch 45mm Leather Link – S/M",
      category: "dây đeo Apple Watch",
      brand: "apple",
      image: "https://shopdunk.com/wp-content/uploads/2022/03/ML7K3.jpg",
      price: "2.890.000",
      oldprice: "",
      sale: "42%"
    },
    {
      id: "17",
      name: "Dây đeo Watch 45mm Nike Sport Band – Regular",
      category: "dây đep Apple Watch",
      brand: "apple",
      image: "https://shopdunk.com/wp-content/uploads/2022/03/AB-1.jpg",
      price: "1.290.000",
      oldprice: "",
      sale: ""
    },
    {
      id: "18",
      name: "Dây đeo Watch 41mm Leather Link – S/M",
      category: "dây đep Apple Watch",
      brand: "apple",
      image: "https://shopdunk.com/wp-content/uploads/2022/03/ML7K3.jpg",
      price: "2.890.000",
      oldprice: "",
      sale: "3%"
    },
    {
      id: "19",
      name: "Dây đeo Watch 41mm Nike Sport Band – Regular",
      category: "dây đep Apple Watch",
      brand: "apple",
      image: "https://shopdunk.com/wp-content/uploads/2022/03/PB.jpg",
      price: "1.290.000",
      oldprice: "",
      sale: ""
    },
    {
      id: "20",
      name: "Dây đeo Watch 45mm Nike Sport Loop – Regular",
      category: "dây đep Apple Watch",
      brand: "apple",
      image: "https://shopdunk.com/wp-content/uploads/2022/03/PB.jpg",
      price: "1.290.000",
      oldprice: "",
      sale: ""
    },
    {
      id: "21",
      name: "Dây đeo Watch 41mm Sport Loop – Regular",
      category: "dây đep Apple Watch",
      brand: "apple",
      image: "https://shopdunk.com/wp-content/uploads/2022/01/ML8F3FE.jpg",
      price: "1.290.000",
      oldprice: "",
      sale: ""
    },
    {
      id: "21",
      name: "Dây đeo Watch S6 44mm Red Sport Band",
      category: "dây đep Apple Watch",
      brand: "apple",
      image: "https://shopdunk.com/wp-content/uploads/2022/01/MYAV.111.jpeg",
      price: "1.290.000",
      oldprice: "1.490.000",
      sale: "13%"
    },
    {
      id: "22",
      name: "Dây đeo Watch S6 40mm Milanese Loop",
      category: "dây đep Apple Watch",
      brand: "apple",
      image: "https://shopdunk.com/wp-content/uploads/2022/01/MYAM.1.jpeg",
      price: "2.450.000",
      oldprice: "",
      sale: "2%"
    },
    {
      id: "23",
      name: "Sạc dự phòng không dây Magsafe Battettery Pack",
      category: "sạc dự phòng",
      brand: "apple",
      image: "	https://shopdunk.com/wp-content/uploads/2022/01/MJWY3.jpg",
      price: "2.490.000",
      oldprice: "2.990.000",
      sale: "17%"
    },
    {
      id: "24",
      name: "Sạc dự phòng Energizer 10000mAh – UE10052",
      category: "sạc dự phòng",
      brand: "energizer",
      image: "https://shopdunk.com/wp-content/uploads/2021/07/e5d788c3aa7c814d629428eb7dc2b8c4.jpeg",
      price: "650.000",
      oldprice: "800.000",
      sale: "19%"
    },
    {
      id: "25",
      name: "Sạc dự phòng Energizer 10000mAh – UE10042",
      category: "sạc dự phòng",
      brand: "energizer",
      image: "https://shopdunk.com/wp-content/uploads/2021/07/7076052636d86c38852cc1c2b0d3b80e.jpeg",
      price: "650.000",
      oldprice: "800.000",
      sale: "19%"
    },
    {
      id: "26",
      name: "Sạc dự phòng Mophie Snap + Juice pack mini 5000 mAh",
      category: "sạc dự phòng",
      brand: "mophie",
      image: "https://shopdunk.com/wp-content/uploads/2021/12/pin-sac-du-phong-mophie-snap-5000mah-6.jpg",
      price: "",
      oldprice: "1.100.000",
      sale: ""
    },
    {
      id: "27",
      name: "RAVPower Sạc dự phòng 6700mAh (RP-PB060)",
      category: "sạc dự phòng",
      brand: "RAVPower",
      image: "	https://shopdunk.com/wp-content/uploads/2021/07/42…o%CC%80ng_6700mAh__RP-PB060_-removebg-preview.png",
      price: "300.000",
      oldprice: "390.000",
      sale: "23%"
    },
    {
      id: "28",
      name: "RAVPower Sạc dự phòng HyperAir 10000mAh 5W (RP-PB081)",
      category: "sạc dự phòng",
      brand: "RAVPower",
      image: "	https://shopdunk.com/wp-content/uploads/2021/07/41…erAir_10000mAh_5W__RP-PB081_-removebg-preview.png",
      price: "650.000",
      oldprice: "750.000",
      sale: "13%"
    },
    {
      id: "29",
      name: "RAVPower Sạc dự phòng chống nước 15000mAh (RP-PB124)",
      category: "sạc dự phòng",
      brand: "RAVPower",
      image: "	https://shopdunk.com/wp-content/uploads/2021/07/37…B%CC%81c_15000mAh__RP-PB124_-removebg-preview.png",
      price: "1.050.000",
      oldprice: "1.090.000",
      sale: "4%"
    },
    {
      id: "30",
      name: "RAVPower Sạc dự phòng 5000mAh (RP-PB098)",
      category: "sạc dự phòng",
      brand: "RAVPower",
      image: "https://shopdunk.com/wp-content/uploads/2021/07/24…o%CC%80ng_5000mAh__RP-PB098_-removebg-preview.png",
      price: "320.000",
      oldprice: "690.000",
      sale: "54%"
    },
    {
      id: "31",
      name: "Giác chuyển đổi MagSafe to MagSafe 2",
      category: "sạc cáp",
      brand: "apple",
      image: "	https://shopdunk.com/wp-content/uploads/2021/12/sac-macbook.png",
      price: "590.000",
      oldprice: "790.000",
      sale: "25%"
    },
    {
      id: "32",
      name: "Cáp USB-C Charge Cable 1m",
      category: "sạc cáp",
      brand: "apple",
      image: "https://shopdunk.com/wp-content/uploads/2021/12/cap-type-c-to-type-c-apple-muf72za-1m.jpg",
      price: "490.000",
      oldprice: "690.000",
      sale: "29%"
    },
    {
      id: "33",
      name: "Sạc 20W USB-C Power Adapter",
      category: "sạc cáp",
      brand: "apple",
      image: "https://shopdunk.com/wp-content/uploads/2021/08/cu%CC%89111.png",
      price: "520.000",
      oldprice: "690.000",
      sale: "25%"
    },
    {
      id: "34",
      name: "Cổng chuyển ADAM casa I4 4 in 1  – Grey",
      category: "sạc cáp",
      brand: "ADAM",
      image: "	https://shopdunk.com/wp-content/uploads/2021/12/hub.png",
      price: "1.350.000",
      oldprice: "1.450.000",
      sale: "7%"
    },
    {
      id: "35",
      name: "Sạc đa cổng Hyperjuice GaN 66W Charger Smallest",
      category: "sạc cáp",
      brand: "hyper",
      image: "https://shopdunk.com/wp-content/uploads/2022/05/sac-66w.png",
      price: "",
      oldprice: "1.390.000",
      sale: ""
    },
    {
      id: "36",
      name: "Sạc 2 cổng HyperJuice 20W Charger Small Size (HJ205)",
      category: "sạc cáp",
      brand: "hyper",
      image: "https://shopdunk.com/wp-content/uploads/2022/04/sac-hp.png",
      price: "",
      oldprice: "450.000",
      sale: ""
    },
    {
      id: "37",
      name: "Cổng chuyển Hyper Drive 4K HDMI 6-in-1 (Gen2)",
      category: "sạc cáp",
      brand: "hyper",
      image: "https://shopdunk.com/wp-content/uploads/2021/09/Untitled-1.jpg",
      price: "",
      oldprice: "2.090.000",
      sale: ""
    },
    {
      id: "38",
      name: "Sạc mophie PD 20W USB-C",
      category: "sạc cáp",
      brand: "mophie",
      image: "https://shopdunk.com/wp-content/uploads/2021/08/f1111.png",
      price: "300.000",
      oldprice: "",
      sale: "39%"
    },
    {
      id: "38",
      name: "Miếng sạc Mophie Snap",
      category: "sạc cáp",
      brand: "mophie",
      image: "https://shopdunk.com/wp-content/uploads/2021/12/631d6ab52235a8451eab0cdce3c0919e.jpg",
      price: "",
      oldprice: "900.000",
      sale: ""
    },
    {
      id: "39",
      name: "Cáp Innostyle Jazzy USB – C to lightning 1.2M",
      category: "sạc cáp",
      brand: "innostyle",
      image: "	https://shopdunk.com/wp-content/uploads/2022/05/day-cap-sac.png",
      price: "",
      oldprice: "400.000",
      sale: ""
    },
    {
      id: "40",
      name: "EarPods with Lightning",
      category: "tai nghe",
      brand: "apple",
      image: "	https://shopdunk.com/wp-content/uploads/2021/08/taiiiiiii.png",
      price: "",
      oldprice: "590.000",
      sale: ""
    },
    {
      id: "41",
      name: "EarPods with Remote",
      category: "tai nghe",
      brand: "apple",
      image: "https://shopdunk.com/wp-content/uploads/2022/01/MD827.jpg",
      price: "350.000",
      oldprice: "790.000",
      sale: "56%"
    },
    {
      id: "42",
      name: "Tai Nghe Beats Flex – All-Day Wireless Earphones",
      category: "tai nghe",
      brand: "beats",
      image: "https://shopdunk.com/wp-content/uploads/2021/06/MYMD2.jpeg",
      price: "1.390.000",
      oldprice: "",
      sale: "30%"
    },
    {
      id: "43",
      name: "Tai Nghe Powerbeats Pro Wireless Earphones",
      category: "tai nghe",
      brand: "beats",
      image: "https://shopdunk.com/wp-content/uploads/2021/06/MXYA2_AV2.jpeg",
      price: "5.990.000",
      oldprice: "",
      sale: "14%"
    },
    {
      id: "44",
      name: "Tai Nghe Powerbeats Wireless Earphones",
      category: "tai nghe",
      brand: "beats",
      image: "https://shopdunk.com/wp-content/uploads/2021/06/MWNX2_AV2.jpeg",
      price: "3.490.000",
      oldprice: "",
      sale: "13%"
    },
    {
      id: "45",
      name: "Trợ lý ảo thông minh – Google Home Mini Chalk US",
      category: "tai nghe",
      brand: "google",
      image: "https://shopdunk.com/wp-content/uploads/2022/04/c8fb6aeffdd68c04e0fc02244111373f.jpg",
      price: "700.000",
      oldprice: "1.000.000",
      sale: "30%"
    },
    {
      id: "46",
      name: "Loa Thông Minh Google Home Mini",
      category: "tai nghe",
      brand: "harman kardon",
      image: "	https://shopdunk.com/wp-content/uploads/2022/04/google-home-mini-gey-full-box-3.jpg",
      price: "650.000",
      oldprice: "1.000.000",
      sale: "35%"
    },
    {
      id: "47",
      name: "Loa Harman Kardon Citation 500",
      category: "tai nghe",
      brand: "harman kardon",
      image: "https://shopdunk.com/wp-content/uploads/2022/01/loa-harman-kardon-citation-500-6.jpg",
      price: "16.800.000",
      oldprice: "",
      sale: ""
    },
    {
      id: "48",
      name: "Loa JBL Pulse 4",
      category: "tai nghe",
      brand: "JBL",
      image: "https://shopdunk.com/wp-content/uploads/2022/01/LOA-HIEU-JBL-PULSE4BLK-1.jpg",
      price: "4.990.000",
      oldprice: "",
      sale: ""
    },
    {
      id: "49",
      name: "Loa JBL PARTYBOXGO Black",
      category: "tai nghe",
      brand: "JBL",
      image: "	https://shopdunk.com/wp-content/uploads/2022/01/loa-jbl-partybox-on-the-go-1.jpg",
      price: "7.450.000",
      oldprice: "8.500.000",
      sale: "12%"
    },
    {
      id: "50",
      name: "Tai nghe JBL Live Pro+ TWS",
      category: "tai nghe",
      brand: "JBL",
      image: "	https://shopdunk.com/wp-content/uploads/2022/01/JB…IVE-PRO_TWS_Product-Image_BACK_Rose-2048x2048.jpg",
      price: "2.990.000",
      oldprice: "",
      sale: ""
    },
    {
      id: "51",
      name: "Apple TV HD 32GB 2020",
      category: "apple TV",
      brand: "apple",
      image: "https://shopdunk.com/wp-content/uploads/2021/08/apple-tv-hd.jpg",
      price: "3.450.000",
      oldprice: "4.990.000",
      sale: "11%"
    },
    {
      id: "52",
      name: "Apple TV 4K 32GB 2021",
      category: "apple TV",
      brand: "apple",
      image: "https://shopdunk.com/wp-content/uploads/2021/08/apple-tv-hd.jpg",
      price: "4.450.000",
      oldprice: "4.990.000",
      sale: "11%"
    },
    {
      id: "53",
      name: "Apple TV 4K 64GB 2021",
      category: "apple TV",
      brand: "apple",
      image: "https://shopdunk.com/wp-content/uploads/2021/08/apple-tv-hd.jpg",
      price: "5.350.000",
      oldprice: "5.990.000",
      sale: "11"
    },
    {
      id: "54",
      name: "Apple TV HD 64GB 2020",
      category: "apple TV",
      brand: "apple",
      image: "https://shopdunk.com/wp-content/uploads/2021/08/apple-tv-hd.jpg",
      price: "5.190.000",
      oldprice: "5.990.000",
      sale: "13%"
    },
    {
      id: "55",
      name: "Điều khiển Apple TV Remote",
      category: "apple TV",
      brand: "apple",
      image: "https://shopdunk.com/wp-content/uploads/2021/06/dieu-khien.png",
      price: "1.750.000",
      oldprice: "1.990.000",
      sale: "12%"
    },
    {
      id: "56",
      name: "Ốp AirTag WIWU SILICONE 4in1",
      category: "airTag",
      brand: "",
      image: "https://shopdunk.com/wp-content/uploads/2021/12/combo-op-airtag-wiwu-4-in-1-silicon-pack-5.jpg",
      price: "149.000",
      oldprice: "299.000",
      sale: "50%"
    },
    {
      id: "57",
      name: "Ốp AirTag WIWU SILICONE LOOP",
      category: "airTag",
      brand: "",
      image: "	https://shopdunk.com/wp-content/uploads/2021/12/op-airtag-wiwu-silicon-loop-orange.jpg",
      price: "149.000",
      oldprice: "299.00",
      sale: "50%"
    },
    {
      id: "58",
      name: "Ốp AirTag 2in1 ESR SILICONE – Black & Blue – ES8210",
      category: "airTag",
      brand: "",
      image: "https://shopdunk.com/wp-content/uploads/2021/12/combo-op-airtag-2in1-esr-silicone-black-red.jpg",
      price: "199.000",
      oldprice: "299.000",
      sale: "33%"
    },
    {
      id: "59",
      name: "Apple AirTag (1 Pack)",
      category: "airTag",
      brand: "",
      image: "https://shopdunk.com/wp-content/uploads/2021/07/apple-airtag-1-pack.2.png",
      price: "690.000",
      oldprice: "922.000",
      sale: "25%"
    },
    {
      id: "60",
      name: "Apple AirTag (4 Pack)",
      category: "airTag",
      brand: "",
      image: "	https://shopdunk.com/wp-content/uploads/2021/07/Untitled-1.png",
      price: "2.190.000",
      oldprice: "3.300.000",
      sale: "34%"
    },
    {
      id: "61",
      name: "Airtag Leather Loop",
      category: "airTag",
      brand: "",
      image: "https://shopdunk.com/wp-content/uploads/2021/06/MX4A2.jpeg",
      price: "790.000",
      oldprice: "",
      sale: "34%"
    },
    {
      id: "62",
      name: "Apple AirTag Leather Key Ring",
      category: "airTag",
      brand: "",
      image: "https://shopdunk.com/wp-content/uploads/2021/06/175127077_749996822345840_5811108778733993453_n.jpeg",
      price: "690.000",
      oldprice: "",
      sale: "30%"
    },
    {
      id: "63",
      name: "AirTag Loop",
      category: "airTag",
      brand: "",
      image: "https://shopdunk.com/wp-content/uploads/2021/06/174785762_172991501355108_6305114184289438606_n.jpeg",
      price: "490.000",
      oldprice: "",
      sale: "43%"
    }
  ],
  service : [
    {
      id: "1",
      name: "Apple Care",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/07/apple-care-logo-03.png",
      price: "690.000",
    },
    {
      id: "2",
      name: "Bảo Hành Kim Cương",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/10/BHKC-02-1-1536x1536.png",
      price: "199.000",
    },
    {
      id: "3",
      name: "Microsoft Office",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2021/07/efc838be9e8aa13b4de4cbfc0b927fd9.1-02-5-692x692.png",
      price: "990.000",
    },
    {
      id: "4",
      name: "Apple School Manager",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/04/Apple-school-manager.png",
      price: "Dành cho Trường học",
    },
    {
      id: "5",
      name: "Apple Business Manager",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/2022/04/Apple-Business-Manager.png",
      price: "Dành cho Doanh nghiệp",
    },
    {
      id: "6",
      name: "Thu cũ đổi mới",
      image: "https://cdn-dgcei.nitrocdn.com/QaFavQVnaqgHtiSsAelwGDKVguOuACXM/assets/static/optimized/rev-ed43433/wp-content/uploads/elementor/thumbs/iphone-trade-in-202010-1-po9osvfx1ow08ys872sup6ecpfq4uhdd6uigd7dxs0.png",
      price: "Xem ngay",
    },
  ]
}

const navbar = [
  {
    id: 1,
    name: "iPhone",
    path: "/iphone"
  },
  {
    id: 2,
    name: "iPad",
    path: "/ipad"
  },
  {
    id: 3,
    name: "mac",
    path: "/mac"
  },
  {
    id: 4,
    name: "apple watch",
    path: "/apple-watch"
  },
  {
    id: 5,
    name: "âm thanh",
    path: "/am-thanh"
  },
  {
    id: 6,
    name: "phụ kiện",
    path: "/phu-kien"
  },
  {
    id: 7,
    name: "dịch vụ",
    path: "/dich-vu"
  },
  {
    id: 8,
    name: "khuyến mại",
    path: "/khuyen-mai"
  },
  {
    id: 9,
    name: "trả góp",
    path: "/tra-gop"
  }
]

const slider = [
  {
    id: 1,
    image: "https://shopdunk.com/wp-content/uploads/2022/05/ip13green_Banner-PC.jpg"
  },
  {
    id: 2,
    image: "https://shopdunk.com/wp-content/uploads/2022/05/ip11ip12_Banner-PC.jpg"
  },
  {
    id: 3,
    image: "https://shopdunk.com/wp-content/uploads/2022/05/vnpay_Banner-PC.jpg"
  },
  {
    id: 4,
    image: "https://shopdunk.com/wp-content/uploads/2022/05/Banner-PC-tang-sim.jpg"
  },
  {
    id: 5,
    image: "https://shopdunk.com/wp-content/uploads/2022/05/Banner-PC-macbook-air-1.jpg"
  },
  {
    id: 6,
    image: "https://shopdunk.com/wp-content/uploads/2022/05/Banner-PC-macbook-Pro-1.jpg"
  }

]

const footer = [

]

export { data, navbar, slider, footer }








